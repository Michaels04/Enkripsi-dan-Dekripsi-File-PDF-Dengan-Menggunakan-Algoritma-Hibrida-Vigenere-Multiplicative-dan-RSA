...
def input_kunci():
    print("===== GENERATOR KUNCI SIMETRIS =====")
    while True:
        try:
            len_key = int(input("Masukkan panjang kunci simetris yang diinginkan : "))
            if len_key <= 0:
                print("Panjang kunci harus lebih dari 0. Silakan coba lagi.")
                continue
            break
        except ValueError:
    ...
    char_key = "".join(printable_character(k) for k in key)
    print("===== KUNCI SIMETRIS TERPILIH (REPRESENTATIF) =====")
